...
class TUI:
    ...
    def gestao_produtos(self):
        while True:
            ...
            table.add_row("2", "Consultar preço de um item")
            table.add_row("3", "Pesquisar produto")
            table.add_row("4", "Ver todos os produtos")
    ...
    def registrar_venda(self):
        id_produto = Prompt.ask("Digite o ID do produto")
        quantidade = Prompt.ask("Digite a quantidade")
        preco_total = Prompt.ask("Digite o preço total")
        forma_pagamento = Prompt.ask("Digite a forma de pagamento (dinheiro, cartão, PIX, boleto)")
        desconto = Prompt.ask("Digite o desconto (se houver)", default="0")
        id_cliente = Prompt.ask("Digite o ID do cliente (se houver)", default="0")
        id_venda = self.sales_manager.registrar_venda(id_produto, quantidade, preco_total, forma_pagamento, desconto, id_cliente)
        self.sales_manager.emitir_recibo(id_venda)
        self.sales_manager.emitir_nota_fiscal(id_venda)
        self.console.print(f"Venda registrada com sucesso! Recibo emitido: recibos/recibo_{id_venda}.pdf")
        self.console.print(f"Nota Fiscal emitida: notas_fiscais/nf_{id_venda}.pdf")
        logging.info(f"Venda registrada: ID Produto: {id_produto}, Quantidade: {quantidade}, Preço Total: {preco_total}, Forma de Pagamento: {forma_pagamento}, Desconto: {desconto}, ID Cliente: {id_cliente}")
    ...
    def ver_todos_produtos(self):
        categoria = Prompt.ask("Digite a categoria do produto (opcional)", default=None)
        produtos = self.product_registration.obter_produtos(categoria)
        table = Table(show_header=True, header_style="bold magenta")
        table.add.column("Nome", justify="center")
        table.add.column("Categoria", justify="center")
        table.add.column("Preço de Custo", justify="center")
    ...
    def ver_movimentacoes(self):
        ...
        for movimentacao in movimentacoes:
            table.add_row(str(movimentacao[0]), str(movimentacao[1]), movimentacao[2], str(movimentacao[3]), movimentacao[4], movimentacao[5])
        self.console.print(table)
    ...
    def ver_horarios(self):
        ...
        for horario in horarios:
            table.add_row(str(horario[0]), horario[1], horario[2], horario[3])
        self.console.print(table)
    ...
    def ver_metas(self):
        ...
        for meta in metas:
            table.add_row(str(meta[0]), meta[1], str(meta[2]))
        self.console.print(table)
