...
class ProductRegistration:
    ...
    def pesquisar_produto(self, nome):
        return [produto for produto in self.produtos if nome.lower() in produto[0].lower()]

    def atualizar_quantidade_produto(self, nome, quantidade):
        for produto in self.produtos:
            if produto[0] == nome:
                produto = list(produto)
                produto[4] = quantidade
                self.produtos[self.produtos.index(produto)] = tuple(produto)
                break